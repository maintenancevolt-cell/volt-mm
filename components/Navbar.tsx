...
export default function Navbar({ locale }: { locale: Locale }) {
  ...
  return (
    ...
    >
      <div className="container-custom">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden lg:flex items-center gap-8">
            <Link href={`/${locale}/`} className="text-sm font-semibold text-gray-700 hover:text-blue-500 transition-colors">
              {tr.nav.home}
            </Link>
            ...
            <Link href={`/${locale}/about/`} className="text-sm font-semibold text-gray-700 hover:text-blue-500 transition-colors">
              {tr.nav.about}
            </Link>
            <Link href={`/${locale}/contact/`} className="text-sm font-semibold text-gray-700 hover:text-blue-500 transition-colors">
              {tr.nav.contact}
            </Link>
          </div>
      ...
      <div className={`fixed inset-0 top-16 md:top-20 z-40 lg:hidden transition-all duration-300 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
          <div className={`absolute inset-0 bg-black/30 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`} onClick={() => setIsOpen(false)} />
          <div className={`absolute top-0 ${locale === 'ar' ? 'right-0' : 'left-0'} w-80 max-w-[85vw] h-full bg-white shadow-2xl overflow-y-auto transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : locale === 'ar' ? 'translate-x-full' : '-translate-x-full'}`}>
            <div className="p-6 space-y-1">
              <Link href={`/${locale}/`} onClick={() => setIsOpen(false)} className="block py-3 px-4 text-base font-semibold text-gray-700 hover:bg-blue-50 hover:text-blue-500 rounded-xl transition-colors">
                {tr.nav.home}
              </Link>
              ...
              <Link href={`/${locale}/about/`} onClick={() => setIsOpen(false)} className="block py-3 px-4 text-base font-semibold text-gray-700 hover:bg-blue-50 hover:text-blue-500 rounded-xl transition-colors">
                {tr.nav.about}
              </Link>
              <Link href={`/${locale}/contact/`} onClick={() => setIsOpen(false)} className="block py-3 px-4 text-base font-semibold text-gray-700 hover:bg-blue-50 hover:text-blue-500 rounded-xl transition-colors">
                {tr.nav.contact}
              </Link>
